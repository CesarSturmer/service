    ...
    <div>
      ...
      {screenOption === 1 && (
        <UserForm
          ...
          back={() => setScreenOption(0)}
        />
      )}
      {screenOption === 2 && <ChangePassword back={() => setScreenOption(0)} />}
      {screenOption === 3 && <ServiceForm back={() => setScreenOption(0)} />}
      {userInfo.length !== 0 && userInfo.perfis.length === 2 && (
        <SubmitButton onClick={() => setScreenOption(3)}>
          Cadastrar serviço
        </SubmitButton>
      )}
